...
def ema(data, period=0, column="Close"):
    ...
    data[f"ema{period}"] = data[column].ewm(span=period).mean()

    return data


def ma(data, period=5, column="Close"):
    """
    Moving average
    Source: http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:moving_averages
    Params:
        data: pandas DataFrame
        period: smoothing period
        column: the name of the column with values for calculating MA in the 'data' DataFrame

    Returns:
        copy of 'data' DataFrame with 'ema[period]' column added
    """

    data[f"ma{period}"] = data[column].rolling(window=period).std()

    return data


def volume(data, period=20, column="Volume"):
    data[f"volume{period}"] = data[column].rolling(window=period).std()
...
def acc_dist(
    ...
    for index, row in data.iterrows():
        if row[high_col] != row[low_col]:
            ac = (
                ((row[close_col] - row[low_col]) - (row[high_col] - row[close_col]))
    ...
    data["acc_dist_ema" + str(trend_periods)] = (
        data["acc_dist"]
        .ewm(ignore_na=False, min_periods=0, com=trend_periods, adjust=True)
        .mean()
    )
...
def on_balance_volume(data, trend_periods=21, close_col="Close", vol_col="Volume"):
    ...
    data["obv_ema" + str(trend_periods)] = (
        data["obv"]
        .ewm(ignore_na=False, min_periods=0, com=trend_periods, adjust=True)
        .mean()
    )
...
def price_volume_trend(data, trend_periods=21, close_col="Close", vol_col="Volume"):
    ...
    data["pvt_ema" + str(trend_periods)] = (
        data["pvt"]
        .ewm(ignore_na=False, min_periods=0, com=trend_periods, adjust=True)
        .mean()
    )
...
def chaikin_oscillator(
    ...
    ema_long = ac.ewm(
        ignore_na=False, min_periods=0, com=periods_long, adjust=True
    ).mean()
    ema_short = ac.ewm(
        ignore_na=False, min_periods=0, com=periods_short, adjust=True
    ).mean()
...
def mass_index(data, period=25, ema_period=9, high_col="High", low_col="Low"):
    high_low = (
        data[high_col] - data[low_col] + 0.000001
    )  # this is to avoid division by zero below
    ema = high_low.ewm(
        ignore_na=False, min_periods=0, com=ema_period, adjust=True
    ).mean()
    ema_ema = ema.ewm(
        ignore_na=False, min_periods=0, com=ema_period, adjust=True
    ).mean()
...
def directional_movement_index(data, periods=14, high_col="High", low_col="Low"):
    ...
    data["di_plus"] = (
        (data["dm_plus"] / data["true_range"])
        .ewm(ignore_na=False, min_periods=0, com=periods, adjust=True)
        .mean()
    )
    data["di_minus"] = (
        (data["dm_minus"] / data["true_range"])
        .ewm(ignore_na=False, min_periods=0, com=periods, adjust=True)
        .mean()
    )
    ...
    data.set_value(0, "dxi", 1.0)
    data["adx"] = (
        data["dxi"].ewm(ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    )
...
def negative_volume_index(data, periods=255, close_col="Close", vol_col="Volume"):
    ...
    data["nvi_ema"] = (
        data["nvi"].ewm(ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    )
...
def positive_volume_index(data, periods=255, close_col="Close", vol_col="Volume"):
    ...
    data["pvi_ema"] = (
        data["pvi"].ewm(ignore_na=False, min_periods=0, com=periods, adjust=True).mean()
    )
...
def chaikin_volatility(
    ...
    data["ch_vol_ema"] = (
        data["ch_vol_hl"]
        .ewm(ignore_na=False, min_periods=0, com=ema_periods, adjust=True)
        .mean()
    )
    data["chaikin_volatility"] = 0.0
    ...
    for index, row in data.iterrows():
        if index >= change_periods:
            ...
            data.set_value(
                index,
                "chaikin_volatility",
                ((row["ch_vol_ema"] - prev_value) / prev_value),
            )
...
def ultimate_oscillator(
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            tr = max(row[high_col], data.at[index - 1, close_col]) - min(
                row[low_col], data.at[index - 1, close_col]
            )

            data.set_value(index, "uo_bp", bp)
            data.set_value(index, "uo_tr", tr)
            if index >= period_1:
                uo_avg_1 = sum(data["uo_bp"][index - period_1 : index]) / sum(
                    data["uo_tr"][index - period_1 : index]
                )
                data.set_value(index, "uo_avg_1", uo_avg_1)
            if index >= period_2:
                uo_avg_2 = sum(data["uo_bp"][index - period_2 : index]) / sum(
                    data["uo_tr"][index - period_2 : index]
                )
                data.set_value(index, "uo_avg_2", uo_avg_2)
            if index >= period_3:
                uo_avg_3 = sum(data["uo_bp"][index - period_3 : index]) / sum(
                    data["uo_tr"][index - period_3 : index]
                )
