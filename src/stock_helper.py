...
def GetReward(reward_cat, trade_log):
    ...
    # profit_win_count win * appt
        ...
        trade_log["reward"] = ((trade_log["profit_win"]) - trade_log["profit_loss"]) * (
            trade_log["win_ratio"] ** 0.5
        )
    elif reward_cat == 8:
        trade_log["reward"] = (
...
def load_all_stock_with_features(stocks, stocks_data):
    ...
    for stock in stocks:
        ...
        stock_data_list[stock] = GetTopFeatures(stock, stocks_data, max_feature=10)
        print(f"\tExtracting features for {stock} was done...")
...
def AddFeatures(data: pd.DataFrame, category: str):
    ...
    if category == "all" or category == "momentum":
        ...
        data = ema(data, period=9)
        data = ema(data, period=20)
        data = ema(data, period=25)
        data = ema(data, period=30)
        data = ema(data, period=35)
        data = ema(data, period=40)
        data = ema(data, period=45)
        data = ema(data, period=50)
        data = ema(data, period=55)
        data = volume(data, period=20)
        ...
        bottom_fish = volume(bottom_fish, period=20)
        data = bottom_fish

    elif category == "macdo":
        indicator_macd = MACD(close=data["Close"], n_slow=20, n_fast=5, n_sign=5)
        data["trend_macd"] = indicator_macd.macd()
        data = ma(data, period=200)
        data = ema(data, period=62)
        data = ema(data, period=38)
        data = volume(data, period=20)
        ...
        data = rsi(data, period=9)
        data = rsi(data, period=25)
        data = rsi(data, period=12)
...
def profit_loss_action(
    ...
    # If action is BUY and has position
        ...
        if (
            ...
            trade_log, trade_list = ExecuteTrade(
                trade_log, position_change, isBackTest=isBackTest, trade_list=trade_list
            )
