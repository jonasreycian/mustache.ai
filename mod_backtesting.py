...
def get_action(data, net):
    ...
    for vals in data.values:
        ...
        trade_log, trade_list = stock_helper.profit_loss_action(
            action, trade_log, trade_list=trade_list, isBackTest=True
        )
    ...
    for stock in stock_list:
        ...
        if stock in ignore_stock:
            ...
            current_stock_data = feature_selection.GetTopFeatures(
                stock, stock_data, max_feature=15, isTrain=False, category=strategy
            )
            run(config_path)
            stock_helper.WriteJSONFile(
                filename=f'analysis_{time.strftime("%m%d%Y")}_20Gen.json',
                data=fitness_analysis,
            )

    trade_list = pd.DataFrame(trade_list, columns=["Date", "Stock", "Price", "Action"])
    trade_list.to_csv(
        (f'result/backtest_result_{time.strftime("%m%d%Y")}.csv'), index=False
    )
