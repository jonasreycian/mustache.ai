...
#       data = data.loc[:'2018-11-05']
    ...
    trade_list.to_csv(("result/trade_list_", time.strftime('%m%d%Y') , ".csv"), index=False)
    trade_list.to_csv(("/mnt/c/daily_trade_result/trade_list_", time.strftime('%m%d%Y') , ".csv"), index=False)
